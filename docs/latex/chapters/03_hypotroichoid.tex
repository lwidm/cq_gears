...
\end{figure}
As described in previous sections we first need to rotate the shift vector by the angle $\phi$ (see Section~\ref{subsec:diff_geom_invo}) and then translate it to the correct position of the construction involute:
...
\begin{equation}
\vb{r_{hypo}}(\phi) = \frac{1}{2}
        \begin{bmatrix}
            d_f\cos(\phi) \mp d_f\tan(\alpha_t)\sin(\phi) + d_p\phi\sin(\phi) \\
            d_f\sin(\phi) \pm d_f\tan(\alpha_t)\cos(\phi) - d_p\phi\cos(\phi) \\
        \end{bmatrix}
\label{eq:hypo_single_matrix}
\end{equation}

\dots as separated terms:
\begin{equation}
\vb{r_{hypo}}(\phi) = \frac{1}{2}\left(
            \begin{bmatrix}
                d_f\\
                \pm d_f\tan(\alpha_t)\\
            \end{bmatrix}
...
\label{eq:hypo_compact_notation}
\end{gathered}
\end{equation}
...
Starting from Eq.~\eqref{eq:hypo_compact_notation} the $x$ and $y$ components are separated:
\begin{subequations}
\begin{align}
x_{hypo}(\phi) &= \frac{1}{2}\left(a\cos\phi - b \sin\phi + d_p\phi\sin\phi\right) \\
y_{hypo}(\phi) &= \frac{1}{2}\left(b\cos\phi + a \sin\phi - d_p\phi\cos\phi\right)
\end{align}
\end{subequations}

With these the diamter can be determined
\begin{subequations}
\begin{align}
...
\label{eq:main_d_star_hypo}
\end{align}
\end{subequations}

Simplifying the terms under the square root in Eq.~\eqref{eq:main_d_star_hypo}:
\begin{subequations}
\begin{align}
...
Inserting into Eq.~\ref{eq:main_d_star_hypo}:
\begin{subequations}
\begin{align}
d^* &= \Big[ 
\nonumber\\
...
\end{equation*}
When dealing with the right flank the construction involute rotates clockwise, therefore wa are interested in the smaller (usually negative) solution for $\phi$. Therefore, the square root term is subtracted.
\begin{equation}
...
b = \textcolor{red}{-}d_f\tan(\alpha_t)
\end{equation*}
When dealing with the left flank the construction involute rotates counterclockwise, therefore wa are interested in th smaller (usually positive) solution for $\phi$. Therefore, the square root term is added.
\begin{equation}
