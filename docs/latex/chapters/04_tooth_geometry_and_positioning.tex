...
To construct a tooth of a gear the two parametric curves described above need to be correctly positioned both relatively to each other but also relatively to the $x$-axis. The curves should be placed (rotated) in such a manner that by mirroring the curves along the $x$-axis a valid tooth geometry emerges with both a right and left flank.

\subsection{Tooth Thickness and Half Tooth Angle, \texorpdfstring{$\gamma$}{γ}}
The tooth thickness (in arc length) at the pitch circle if there is \textbf{no profile shift} is:
\begin{equation}
    s_0 = \frac{m\pi}{2}
    \label{eq:tooth_thickness_pitch}
\end{equation}

Therefore the angle spanned by half of the tooth at the pitch circle is
\begin{equation}
    \gamma_0 = \frac{m\pi}{2\;d_p}
\end{equation}

To get the angle spanned by the half of the tooth at the base circle we need to add the angle spanned by the involute at $\phi(d_p)$.
...
This results in a half tooth angle at the base of:

\begin{subequations}
\begin{align}
    \gamma &= \gamma_0+\Delta \gamma \\
           &= \frac{m\pi}{2\;d_p} + \left[\left(\frac{d_p}{d_b}\right)^2 - 1\right]^{\tfrac{1}{2}} - \arctan\left(\left[\left(\frac{d_p}{d_b}\right)^2 - 1\right]^{\tfrac{1}{2}}\right)
\end{align}
\end{subequations}
...
Due to the current definition of the undercut curve (see Figure~\ref{fig:undercut_video_screenshot} the undercut curve needs to be rotated by both the half tooth angle at the base circle as well as the transversal pressure angle ($\alpha_t$).
