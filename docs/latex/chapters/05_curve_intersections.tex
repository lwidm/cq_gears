...
\end{align}
By looking at how the two curves are defined (see Figures~\ref{fig:involute_construction}~and~\ref{fig:undercut_generated_video}) one cas see that the involute needs to be rotated by $\mypm(-\gamma)$ and the undercut curve needs to be rotated by $\mypm(-\gamma - \alpha_t)$. When finding the intersection this is equivalent to rotating the involute by $\mypm \alpha_t$.
...
\begin{equation}
    \vb{r_{inv,pos}}(\phi_i) = \vb{r_{undercut}}(\phi_h)
\end{equation}
\begin{equation}
      \begin{aligned}
      & \frac{d_b}{2}\begin{bmatrix}
      ...
      \end{bmatrix}
      \\
      &\hspace{10em} =\frac{1}{2}\left(\begin{bmatrix}a \\ b\end{bmatrix}\cos\phi_h + \begin{bmatrix}
   -b \\ a\end{bmatrix}\sin\phi_h + d_p\phi_h\begin{bmatrix} \sin\phi_h \\ -
  \cos\phi_h\end{bmatrix}\right)
      \end{aligned}
\end{equation}
\begin{equation}
    d_b\begin{bmatrix}
    ...
    =
    \begin{bmatrix}
        a \cos(\phi_h) - b\sin(\phi_h)+ d_p\phi_h\sin(\phi_h)\\
        b\cos(\phi_h) + a \sin(\phi_h) - d_p\phi_h\cos(\phi_h)
    \end{bmatrix}
\end{equation}
